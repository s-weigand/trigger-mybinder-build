...
var original = __webpack_require__(440)
 ...
 * @param serviceName
 */
function validServiceName(serviceName) {
    return serviceNames.includes(serviceName);
}
exports.validServiceName = validServiceName;
    ...
    source.onmessage = ((event) => {
        ...
        if (checkDone(startTime, timeOut, eventData)) {
            if (['launching', 'ready'].indexOf(eventData.phase) > -1) {
                ...
                source.close();
                throw new Error(`Your binder build failed with the following
          message:\n${eventData.message}`);
            }
            source.close();
        }
